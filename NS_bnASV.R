...
pdist <- phydist[species,species] # = NTD
rm(phydist)
...
NTD_calc <- function(i,j,spp,y,pdist,OTU){
  ...
  if (y[i]>0 & y[j]==0){
    ...
    min.dist <- names(which.min(pdist[spp,other.species]))
    a <- y[i]
  } else if (y[j]>0 & y[i]==0){
    other.species <- setdiff(colnames(pdist),spp)
...
dsim <- readRDS(file="nucsim_mothur.RDS") # load nucleotide similarity matrix

NTD_data$ns <- rep(NA,nrow(NTD_data)) #create empty column
for (i in 1:nrow(NTD_data)) {
  NTD_data$ns[i] <- dsim[NTD_data$species[i],NTD_data$distances[i]]
}

setkey(NTD_data,species,community_index)

ntd_spp <- NTD_data[,list(mean_ns=mean(ns)), by='species'] 

saveRDS(ntd_spp,file="NucSim_bnearestASV.RDS")
