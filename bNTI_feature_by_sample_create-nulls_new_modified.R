## Modification of bNTI_feature_by_sample_create-nulls_new.R code
## Original code at https://github.com/danczakre/betaNTI-feature
...
rm.tax = F # This configures whether the assemblage data has taxonomy
range = 1:100 # Setting replicate numbers
...
path_to_wd = "path_to_your_working_directory"
setwd(path_to_wd)

library(phyloseq)
pseq_rarefy <- readRDS("pseq_rarefy.RDS")
data = as.data.frame(otu_table(pseq_rarefy)) # Load assemblage
tree = phy_tree(pseq_rarefy) # Load tree
...
# Not removing low sequence ASVs in our analysis (Quiroga et al. 2023)
...
coph[which(coph==0)] <- NA
coph[which(is.na(coph))] <- head(sort(coph))[1]
diag(coph)<-0
#--------------------------
...
for(rep in range){
    ...
    for (j in (i + 1):samp.num) {  
      ...
      if(rm.conspec){
        ...
        if(length(which(duplicated(null.dist$Names) %in% TRUE)) > 0){
          if(sum(null.dist$Dist[duplicated(null.dist$Names)]) == 0){  
            null.dist = null.dist[!(duplicated(null.dist$Names) | duplicated(null.dist$Names, fromLast = TRUE)),]
            } else {
      ...
      # line 153: removing all community pair comparisons where the focal ASV was present in both samples 
      
      # Creating an object with all community members to merge in those which were present in these two samples
  ...
  for(i in 1:samp.num){
    ...
    null.by.samp[,i] = rowMeans(temp, na.rm = T)
    
    # Clean-up
    rm(temp, curr.mem, curr.samp)
  }
  #
  if(!dir.exists(paste(path_to_wd, "/", Sample_Name, sep = ""))){
    dir.create(paste(path_to_wd, "/", Sample_Name, sep = ""))
  }
  
  write.csv(null.by.samp, paste0(path_to_wd, "/", Sample_Name, "/", Sample_Name, "_Feature_Samp_Null_rep", rep, ".csv"),
            quote = F, row.names = T)
