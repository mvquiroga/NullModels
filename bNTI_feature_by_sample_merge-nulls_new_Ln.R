## Modification of bNTI_feature_by_sample_merge-nulls_new.R code
...
files = list.files(path = paste0(path_to_wd, "/", Sample_Name),
                  pattern = "Feature_Samp_Null", full.names = T) # Listing files WS
...
for (i in 1:(samp.num - 1)) {
  for (j in (i + 1):samp.num) {
    ...
    if(rm.conspec){
      ...
      if(length(which(duplicated(min.dist$Names) %in% TRUE)) > 0){
        if(mean(min.dist$Dist[duplicated(min.dist$Names)]) == 0){
          min.dist = min.dist[!(duplicated(min.dist$Names) | duplicated(min.dist$Names, fromLast = TRUE)),]
          } else {
    ...
    # line 163: removing all community pair comparisons where the focal ASV was present in both samples 
...
for(i in 1:samp.num){
  ...
  min.by.samp[,i] = rowMeans(temp, na.rm = T)
...
for(i in 1:ncol(null.by.samp)){
  for(j in 1:nrow(null.by.samp)){
    ...
    feat.by.samp[j,i] = ((log(min.by.samp[j,i])-mean(log(m), na.rm=T))/sd(log(m), na.rm=T)) # The bNTI calculation
  }
} # Line 239: natural logarithm transformation    
...
p = ggtree(tree)
p = facet_plot(p, panel = "AF CF RP Ln 100rep", data = feat.samp.frame, 
